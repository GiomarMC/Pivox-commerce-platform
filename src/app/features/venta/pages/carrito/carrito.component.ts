...
@Component({
  ...
  styles: [`
    ...
    .cart-footer { position:fixed; bottom:0; left:0; right:0; background:#fff; border-top:1px solid #EEF1F6; padding:1rem; z-index:20; }
    .cart-total-box { display:flex; align-items:center; justify-content:space-between; background:#F0F2FF; border-radius: 16px; padding:0.75rem 1rem; margin-bottom:0.875rem; }
    ...
    @if (svc.items().length > 0) {
      <div class="cart-footer">
        <div class="cart-total-box">
          <div>
            <p style="font-size:0.72rem;color:#64748B;margin:0 0 0.1rem">{{ svc.items().length }} producto(s)</p>
            <p style="font-size:0.8rem;font-weight:600;color:#334155;margin:0">Total a pagar</p>
          </div>
          <p style="font-size:1.35rem;font-weight:800;color:#334155;margin:0;letter-spacing:-0.02em">S/ {{ svc.total() | number:'1.2-2' }}</p>
        </div>
