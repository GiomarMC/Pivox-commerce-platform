...
@Component({
  ...
  standalone: true,
  styles: [`
    .tv-wrap { min-height: 100dvh; background: #F8FAFC; display: flex; flex-direction: column; }
    .tv-header { background: #fff; border-bottom: 1px solid #EEF1F6; padding: 1rem; }
    .tv-body { flex: 1; padding: 1rem; max-width: 480px; margin: 0 auto; width: 100%; display: flex; flex-direction: column; gap: 0.75rem; }
    .tv-card { width: 100%; text-align: left; padding: 1rem; border-radius: 16px; border: 2px solid #EEF1F6; background: #fff; cursor: pointer; font-family: inherit; transition: border-color 0.15s, box-shadow 0.15s; display: flex; align-items: flex-start; gap: 0.875rem; }
    .tv-card:hover { border-color: #334155; box-shadow: 0 2px 8px rgba(31,42,124,0.1); }
    .tv-card-active { border-color: #334155; box-shadow: 0 2px 8px rgba(31,42,124,0.12); }
    .tv-icon { width: 40px; height: 40px; border-radius: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .tv-card-label { font-size: 0.95rem; font-weight: 700; color: #334155; margin: 0 0 0.2rem; line-height: 1.2; }
    .tv-card-desc { font-size: 0.78rem; color: #64748B; margin: 0; line-height: 1.4; }
    .tv-in-progress { background: #EEF2FF; border: 1px solid #C7D2FE; border-radius: 16px; padding: 0.875rem; display: flex; flex-direction: column; gap: 0.5rem; }
    .tv-in-progress-title { font-size: 0.82rem; font-weight: 700; color: #3730A3; margin: 0; }
    .tv-in-progress-subtitle { font-size: 0.75rem; color: #4F46E5; margin: 0; }
    ...
    @media (min-width: 768px) {
      ...
      .tv-cards .tv-card svg[stroke="#334155"] { align-self: flex-end; }
    }
  `],
  template: `
    <div class="tv-wrap">
      <div class="tv-header">
        <h1 style="font-size:1.1rem;font-weight:800;color:#334155;margin:0">Nueva venta</h1>
        <p style="font-size:0.8rem;color:#94A3B8;margin:0.15rem 0 0">Selecciona el tipo de venta para continuar</p>
      </div>
      ...
      <div class="tv-body">
        ...
        <p style="font-size:0.72rem;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:0.06em;margin:0.25rem 0 0">Tipo de venta</p>

        <div class="tv-cards">

        <!-- Venta Normal -->
        <button type="button" (click)="seleccionar('NORMAL')" [class]="'tv-card' + (tipoActual() === 'NORMAL' ? ' tv-card-active' : '')">
          <div class="tv-icon" style="background:#ECFDF5;color:#10B981">
          ...
          @if (tipoActual() === 'NORMAL') {
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#334155" stroke-width="2.5" style="flex-shrink:0;margin-top:2px">
        ...
        <button type="button" (click)="seleccionar('CREDITO')" [class]="'tv-card' + (tipoActual() === 'CREDITO' ? ' tv-card-active' : '')">
          <div class="tv-icon" style="background:#E0F2FE;color:#6366F1">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <button type="button" (click)="seleccionar('SUNAT')" [class]="'tv-card' + (tipoActual() === 'SUNAT' ? ' tv-card-active' : '')">
          <div class="tv-icon" style="background:#FEF2F2;color:#EF4444">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
