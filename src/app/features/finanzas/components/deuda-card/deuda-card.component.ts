...
@Component({
  ...
  standalone: true,
  styles: [`
    .dc-card { background:#fff; border:1px solid #E2E6F0; border-radius:14px; padding:1rem; }
    .dc-mono-row { display:flex; justify-content:space-between; align-items:center; font-size:0.75rem; padding:0.2rem 0; }
    .dc-progress { width:100%; background:#F3F4F6; border-radius:99px; height:5px; margin-top:0.5rem; overflow:hidden; }
    ...
    <div class="dc-card">
      <div style="display:flex;gap:0.75rem;align-items:flex-start">
        ...
        <div style="width:44px;height:44px;border-radius:12px;flex-shrink:0;display:flex;align-items:center;justify-content:center"
          [style.background]="deuda().tipoOrigen === 'venta' ? '#EFF6FF' : '#F0FDF4'">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="margin-top:0.5rem">
            <div class="dc-mono-row">
              ...
              <span style="font-weight:500;color:#374151">S/ {{ deuda().montoTotal }}</span>
            </div>
            <div class="dc-mono-row">
              <span style="color:#9CA3AF">Pagado</span>
              <span style="font-weight:500;color:#16A34A">S/ {{ montoPagado() }}</span>
            </div>
            <div class="dc-mono-row" style="font-weight:700">
              <span style="color:#374151">Saldo pendiente</span>
              <span style="color:#DC2626">S/ {{ deuda().saldo }}</span>
            </div>
          </div>
          ...
          <div class="dc-progress">
            <div class="dc-bar" [style.width.%]="porcentajePagado()"
              [style.background]="deuda().estado === 'PAGADA' ? '#16A34A' : '#1F2A7C'"></div>
          </div>
        </div>
      </div>
      ...
      @if (deuda().pagos.length > 0) {
        <div style="margin-top:0.75rem;padding-top:0.75rem;border-top:1px solid #F2F4FA">
          <p style="font-size:0.72rem;color:#9CA3AF;margin:0 0 0.5rem">Pagos anteriores</p>
          @for (p of deuda().pagos; track p.fecha) {
            <div class="dc-mono-row">
              <span style="color:#9CA3AF">{{ p.fecha }}</span>
              <span style="font-weight:500;color:#374151">S/ {{ p.monto }}</span>
            </div>
          }
        </div>
      }
